...
import { createQuestion as createQuestionExamApi } from "@/api/exam";
...
export function useQuestionSubmit({
  ...
  const handleSaveAndNew = useCallback(async () => {
    ...
    const response = await createQuestionExamApi(
      sectionId,
      buildPayload(tasks),
    );
    ...
    handleApiResponse(response.statusCode, response.message, () => {
      ...
      const lastGroup = tasks.questionGroup[tasks.questionGroup.length - 1];
      setTasks(
        createQuestionGroupList(QUESTION_TYPE.MCQ, lastGroup.serialNumber + 1),
      );
    });
  }, [validate, sectionId, tasks, handleApiResponse, setAlert, setTasks]);
...
export default function AddQuestionBank() {
  ...
  const { sectionId, returnPath, index } = location.state as LocationState;

  const [tasks, setTasks] = useState<QuestionGroupListRequest>(() =>
    createQuestionGroupList(QUESTION_TYPE.MCQ, index),
  );
  ...
  const { handleSaveAndNew, handleSubmit, handleGetFileUrl } =
    useQuestionSubmit({
      sectionId,
      tasks,
      returnPath,
    ...
    <Box sx={{ width: "100%", p: 5 }}>
      <form onSubmit={handleSubmit} noValidate>
        ...
        <ConfirmDialog
          ...
          onConfirm={() => navigate(returnPath, { state: { tabValue: 3 } })}
        />

        <Box display="flex" alignItems="center" gap={1} mb={2}>
          <Typography variant="h4">Thêm câu hỏi vào phần thi</Typography>
        </Box>
